# W1 Gate Fix Stop-Hook Verification 3
...
- Latest prior W1 verification commit: `f4b2de913 docs(pty): record W1 gate fix stop-hook verification retry`

## Evidence

| Criterion | Invocation | Result | Evidence |
|---|---|---|---|
| Current state and committed W1 artifacts | `git log -5 --oneline`; `git status --short`; `git show --name-status --oneline --summary 04e55140e`; `git show --name-status --oneline --summary f4b2de913` | W1 implementation and prior verification commits are present; unrelated untracked evidence remains in the shared worktree | `01-state-and-commits.txt` |
...
| Required W1 evidence presence | `test -s` and `git ls-tree` checks | W1 matrix, slop review, cleanup receipt, and prior retry summary are present and tracked | `03-evidence-presence.txt` |
...
| Public wrapper lifecycle | `npx tsx /tmp/w1-stop-hook-public-pty-probe-3.mts` | `backend: native`; output includes `senpi-stop-hook-native-probe-3`; kill ok; exit cancelled and not timed out | `10-node-public-create-terminal-session.txt` |
...
The W1 gate-fix claim is verified for a third time from the current checkout. The native-core file split satisfies the pure LOC ceiling, all required command gates pass, the darwin-arm64 prebuild is fresh, and both raw native and public terminal-session lifecycles work through captured CLI stdout probes.

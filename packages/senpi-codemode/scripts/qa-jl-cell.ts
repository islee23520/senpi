...
async function runQa(options: QaOptions): Promise<void> {
	const detected = await createInterpreterDetector().detect("jl");
	if (!detected.ok) throw new QaUsageError("No jl interpreter is available");
	...
	try {
		const kernel = JuliaKernel.start({
			...
			sessionId: "qa-jl-" + crypto.randomUUID(),
			cwd: options.cwd,
		...
		try {
			for (const [index, code] of options.codes.entries()) {
				await kernel.run({
					cellId: "qa-cell-" + String(index + 1),
					code,
					timeoutMs: index === 0 ? 60_000 : 15_000,
				});
			}
		} finally {
			await kernel.close();

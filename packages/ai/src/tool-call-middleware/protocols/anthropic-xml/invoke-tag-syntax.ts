...
const ANTML_NAMESPACE = "antml:";
const OPTIONAL_ANTML_NAMESPACE = "(?:antml:)?";
const INVOKE_OPEN_TAG_PREFIX = String.raw`<\s*${OPTIONAL_ANTML_NAMESPACE}invoke\b\s+name\s*=\s*`;
const INVOKE_OPEN_TAG_BODY = `${INVOKE_OPEN_TAG_PREFIX}(?:"([^"]+)"|'([^']+)')`;
const INVOKE_OPEN_TAG = new RegExp(`${INVOKE_OPEN_TAG_BODY}\\s*>`, "g");
const INVOKE_CLOSE_TAG = new RegExp(String.raw`<\s*\/\s*${OPTIONAL_ANTML_NAMESPACE}invoke\s*>`, "g");
const NESTED_INVOKE_OPEN_TAG = new RegExp(String.raw`<\s*${OPTIONAL_ANTML_NAMESPACE}invoke\b[^>]*>`, "g");
const PARAMETER_MARKUP = new RegExp(String.raw`<\s*\/?\s*${OPTIONAL_ANTML_NAMESPACE}parameter\b`, "g");
const PARAMETER_OPEN_TAG = new RegExp(
	String.raw`<\s*${OPTIONAL_ANTML_NAMESPACE}parameter\b\s+name\s*=\s*(?:"([^"]+)"|'([^']+)')\s*>`,
	"g",
);
const PARAMETER_CLOSE_TAG = new RegExp(String.raw`<\s*\/\s*${OPTIONAL_ANTML_NAMESPACE}parameter\s*>`, "g");
...
export function findParameterOpenTagAt(text: string, index: number): ParameterOpenTagMatch | null {
...
export function findParameterBoundary(text: string, fromIndex: number): ParameterBoundary | null {
...
export function findIncompleteInvokeOpenTag(text: string, fromIndex: number): InvokeOpenTagMatch | null {
	...
	const match = new RegExp(`^${INVOKE_OPEN_TAG_PREFIX}`).exec(candidate);
	if (!match) {
		return null;
	}
...
export function scanInvokeBlock(text: string, openingTag: InvokeOpenTagMatch): InvokeBlockMatch | null {
	...
	while (cursor <= text.length) {
		...
		if (!parameterMarkup || invokeClose.index < parameterMarkup.index) {
			return { contentEnd: invokeClose.index, end: invokeClose.index + invokeClose.length, parameters };
		}

		const parameterOpen = findParameterOpenTagAt(text, parameterMarkup.index);
		if (!parameterOpen) {
			return { contentEnd: invokeClose.index, end: invokeClose.index + invokeClose.length, parameters: null };
		}
...
export function findParameterMarkup(text: string, fromIndex: number): TagMatch | null {
	return findTag(PARAMETER_MARKUP, text, fromIndex);
}

export function isWhitespaceOrInvokeClosePrefix(remainder: string): boolean {
	const compactRemainder = remainder.replace(/\s/g, "");
	return (
		compactRemainder.length === 0 ||
		["</invoke>", "</antml:invoke>"].some(
			(prefix) => prefix.startsWith(compactRemainder) && compactRemainder !== prefix,
		)
	);
}
...
export function isPotentialProtocolStart(candidate: string): boolean {
	...
	if (namePart.length === 0 || ANTML_NAMESPACE.startsWith(namePart)) {
		return true;
	}

	for (const namespace of ["", ANTML_NAMESPACE]) {
		if (!namePart.startsWith(namespace)) {
			continue;
		}
		const unqualifiedPart = namePart.slice(namespace.length);
		const tagName = ["invoke", "parameter", "function_calls"].find(
			(name) => name.startsWith(unqualifiedPart) || unqualifiedPart.startsWith(name),
		);
		if (!tagName) {
			continue;
		}
		if (unqualifiedPart.length < tagName.length) {
			return true;
		}

		const remainder = unqualifiedPart.slice(tagName.length);
		if (remainder.includes(">")) {
			return false;
		}
		return tagName === "function_calls" ? /^\s*$/.test(remainder) : isAttributePrefix(remainder);
	}

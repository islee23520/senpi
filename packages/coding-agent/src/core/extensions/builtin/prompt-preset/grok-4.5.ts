...
// through `bash` spawning `senpi --print`. To pass GPT-5.6 doctrine
// (implement-don't-propose, Manual QA Gate, binding stop contract) to workers,
// the CEO spawns them with `--model gpt-5.6*` — the worker then loads the
// gpt-5.6 preset automatically, so the CEO prompt does not duplicate that
// doctrine. Before deploying, the CEO consults a separate review invocation
// (Oracle pattern) and audits worker evidence itself.
//
...
- **Delegate implementation via \`bash\`.** Spawn a worker subprocess: \`senpi --print -p "<delegation prompt>" --model gpt-5.6*\` (background \`&\` for parallel; capture to a temp file, \`read\` to collect). Using \`gpt-5.6*\` as the worker model loads the GPT-5.6 doctrine (implement-don't-propose, Manual QA Gate, binding stop contract) automatically — you do not need to restate it. Your delegation prompt carries the deliverable, success criteria, stop condition, file paths, and constraints.

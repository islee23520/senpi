...
export function renderWebfetchCall(args: unknown, theme: Theme): Text {
  ...
  const timeout =
    webfetchArgs.timeout === undefined
      ? ""
...
export function renderWebfetchResult(
  ...
  const statusKey =
    details.status >= 200 && details.status < 300 ? "success" : "warning";
  ...
  const truncatedNote = details.outputTruncated
...
function collectNonEmptyTrimmedLines(text: string, limit: number): string[] {
  ...
  while (lines.length < limit && start <= text.length) {
    const newlineIndex = text.indexOf("\n", start);
    const line = (
      newlineIndex === -1 ? text.slice(start) : text.slice(start, newlineIndex)
    ).trim();
